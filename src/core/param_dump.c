...
#define COL_VALUE_W  25  /**< 值列: TYPE=VALUE */
                     ...
                     char *line, uint16_t size)
{
    ...
    col_fill(c_vl, COL_VALUE_W, "UINT=%lu(0x%04X)", (unsigned long)v.u32, (unsigned)v.u32 & 0xFFFF);
    if (re->has_range)
                     ...
                     char *line, uint16_t size)
{
    ...
    uint16_t show = be->blob_size < 6u ? be->blob_size : 6u;
    char bl_buf[24];
...
/**
 * @brief IP 参数格式化 — 优先用 App 同型格式化器，fallback 为通用 IP 格式
 *
 * IP 参数的 UINT/INT/FLOAT/BOOL 类型与 App 共享相同的格式化器。
 * 未能匹配的类型的用通用 IP=val 格式。
 */
                   ...
                   char *line, uint16_t size)
{
    ...
    if (t < PARAM_TYPE_COUNT && g_dump_formatters[t]) {
        ...
        col_fill(c_id, COL_ID_W, " [%04X]", (unsigned)e->param_id & 0xFFFF);
        col_fill(c_nm, COL_NAME_W, "%s", name);
        col_fill(c_vl, COL_VALUE_W, "IP=%lu(0x%04X)", (unsigned long)v.u32, (unsigned)v.u32 & 0xFFFF);
        col_empty(c_rg, COL_RANGE_W);
        col_fill(c_dt, COL_DIRTY_W, "d=%u", dirty);
        col_fill(c_fl, COL_FLAGS_W, "f=%s", flags_to_str(flags));
