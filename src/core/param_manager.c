...
static param_entry_t *hash_find(uint32_t param_id)
{
    ...
    for (uint16_t i = 0; i < PARAM_HASH_SIZE; i++) {
        ...
        if (!e) return NULL;
        if (e->param_id == param_id) return e;
        slot = (slot + 1) & (PARAM_HASH_SIZE - 1);
    }
    return NULL;
}
...
{
    if (hash_find(entry->param_id)) return true;
...
static inline uint16_t get_module_order_index(uint16_t module_id)
{
    for (uint16_t i = 0; i < MODULE_ORDER_COUNT; i++) {
        if (g_module_order[i] == module_id) return i;
    }
 ...
 * @brief 初始化参数管理框架
 *
 * 如果已初始化则返回 PARAM_ERR_BUSY。
 * 调用 storage->init() 完成底层存储初始化。
 *
 * @param storage 持久化后端驱动 (可为 NULL)
 * @return PARAM_OK 或错误码
 */
int param_init(const param_storage_drv_t *storage)
{
    if (g_pm.initialized) return PARAM_ERR_BUSY;

    memset(&g_pm, 0, sizeof(g_pm));
    g_pm.storage = storage;

    if (storage && storage->init) {
        int ret = storage->init(storage->ctx);
        if (ret != 0) return PARAM_ERR_FLASH_FAIL;
    }
...
int param_module_register(param_module_t *module,
                          ...
                          uint16_t count)
{
    if (!g_pm.initialized) return PARAM_ERR_NOT_FOUND;
    if (!module || !entries || count == 0) return PARAM_ERR_INVALID_ID;
...
int param_write(uint32_t param_id, param_value_t value)
{
    ...
    if (!e) { UNLOCK(); return PARAM_ERR_INVALID_ID; }
...
{
    if (!g_pm.initialized) return PARAM_ERR_NOT_FOUND;
    if (!data || len == 0) return PARAM_ERR_INVALID_ID;
    LOCK();
    param_entry_t *e = find_entry(param_id);
    if (!e) { UNLOCK(); return PARAM_ERR_INVALID_ID; }
...
{
    if (!g_pm.initialized) return PARAM_ERR_NOT_FOUND;
    if (!value) return PARAM_ERR_INVALID_ID;
    LOCK();
    param_entry_t *e = find_entry(param_id);
    if (!e) { UNLOCK(); return PARAM_ERR_INVALID_ID; }
...
int param_exec(uint32_t cmd_id, void *user_arg)
{
    ...
    if (!e || !(entry_flags(e) & PARAM_FLAG_EXEC)) { UNLOCK(); return PARAM_ERR_NOT_FOUND; }
    param_module_node_t *m = param_module_find(PARAM_MODULE_ID(cmd_id));
    if (!m) { UNLOCK(); return PARAM_ERR_NOT_FOUND; }
...
{
    for (uint16_t i = 0; i < count; i++)
        if (order[i] == module_id) return true;
    return false;
}
...
{
    if (!g_pm.initialized) return PARAM_ERR_NOT_FOUND;
    if (!g_pm.storage || !g_pm.storage->save) return PARAM_ERR_NOT_FOUND;
...
{
    if (!g_pm.initialized) return PARAM_ERR_NOT_FOUND;
    if (!g_pm.storage || !g_pm.storage->load) return PARAM_ERR_NOT_FOUND;
 ...
 * @brief 重置所有参数为默认值
 *
 * 两阶段: (1) 遍历哈希表重置所有 entry 的缓存值 (2) 遍历模块链表重置模块状态 (dirty/dirty_map)。
 * @return PARAM_OK
 */
int param_reset_all(void)
{
    ...
    /* 第二阶段: 重置模块状态 (dirty / dirty_map 等) */
    param_module_node_t *m = g_pm.module_head;
    while (m) {
        if (m->vtable && m->vtable->reset)
            m->vtable->reset(m);
        m = m->next;
    }
...
/** @brief 重置指定参数为默认值，同时清除对应的 dirty 标记 */
...
{
    if (!g_pm.initialized) return;
    if (stats) { LOCK(); *stats = g_pm.stats; UNLOCK(); }
}
    ...
    while (m) {
        if (module_id == 0 || m->module_id == module_id) {
            for (uint16_t i = 0; i < m->param_count; i++) {
                ...
                if (!e) continue;
                if (!cb(e, user_data)) { UNLOCK(); return; }
            }
        }
        m = m->next;
    }
                    ...
                    const param_value_t *max_val)
{
    ...
    LOCK();
    param_entry_t *e = find_entry(param_id);
    if (!e || !is_app(e)) { UNLOCK(); return PARAM_ERR_INVALID_ID; }
    ...
    if (min_val) re->min = *min_val;
    if (max_val) re->max = *max_val;
    re->has_range = 1;
    app_clamp_entry(e);
