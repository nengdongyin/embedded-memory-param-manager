...
| `param_module` | ~48B | .data | APP 模块 (含回调) |
...
- `param_set_range`: 运行时修改参数的范围，仅对 APP 的 UINT/INT/FLOAT 生效。`NULL` 表示不修改对应边界。
...
在 `core/module_ids.h` 中追加 (0x01~0x7F 为建议的 APP 区段，0x80~0xFF 为建议的 IP 区段):

```c
#define MODULE_PID_CTRL  0x02u  // APP 模块 0x01~0x7F
...
| APP_param_manager.c | ~580 | ~3.3 KB |
...
| 每 APP 模块 | ~48B | 含链表节点 + 回调指针 |
| 每 IP 实例 | ~80B | 含链表节点 + driver 回调 + 64bit dirty_map |
| 每 APP entry (ROM) | 20~32B | 根据类型不同 (保存在 .rodata) |
