...
IP_DRIVER_DEFINE(order_ip, TEST_MODULE_IP, "OrderIP", NULL, NULL, mock_ip_write_ok);
...
void test_init_normal(void) {
    ...
    TEST_ASSERT_EQUAL_INT(1, g_mock_init_called);
}
...
void test_init_storage_fails(void) {
    param_deinit();
    mock_storage_reset();
    mock_storage_set_init_ret(-1);
    int ret = param_init(&g_mock_storage);
...
void run_test_init_order(void)
{
    ...
    RUN_TEST(test_init_storage_fails);
