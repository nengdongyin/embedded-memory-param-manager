...
{
    if (g_mock_kv_count >= MOCK_STORAGE_MAX) return;
    if (len > 64) return;
...
static mock_kv_t *mock_storage_find(uint32_t id)
{
    for (uint16_t i = 0; i < g_mock_kv_count; i++) {
        if (g_mock_kv[i].id == id) return &g_mock_kv[i];
    }
    return NULL;
}

static int mock_storage_init(void *ctx)
{
    (void)ctx;
    g_mock_init_called++;
...
static int mock_storage_load(void *ctx, uint32_t id, uint8_t *data, uint16_t len)
{
    (void)ctx;
    if (g_mock_load_ret != 0) return g_mock_load_ret;
    ...
    if (copy_len < len) memset(data + copy_len, 0, len - copy_len);
    return 0;
}
...
{
    (void)ctx;
    if (g_mock_save_ret != 0) return g_mock_save_ret;

    mock_kv_t *kv = mock_storage_find(id);
    if (!kv) {
        if (g_mock_kv_count >= MOCK_STORAGE_MAX) return -1;
...
static mock_kv_t *mock_storage2_find(uint32_t id)
{
    for (uint16_t i = 0; i < g_mock_kv2_count; i++) {
        if (g_mock_kv2[i].id == id) return &g_mock_kv2[i];
    }
    return NULL;
}

static int mock_storage2_init(void *ctx)
{
    (void)ctx;
    g_mock_init_called2++;
    return 0;
...
{
    (void)ctx;
    if (g_mock_save_ret2 != 0) return g_mock_save_ret2;

    mock_kv_t *kv = mock_storage2_find(id);
    if (!kv) {
        if (g_mock_kv2_count >= MOCK_STORAGE_MAX) return -1;
...
static int mock_storage2_erase_all(void *ctx) { (void)ctx; return 0; }
static int mock_storage2_deinit(void *ctx)   { (void)ctx; return 0; }

param_storage_drv_t g_mock_storage2 = {
    .ctx       = NULL,
    .init      = mock_storage2_init,
